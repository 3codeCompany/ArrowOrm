...
class PersistentFactory
{
    ...
    public static function save(PersistentObject $object, $fireSaveEvents = true, $forceCreate = false)
    {
        ...
        if ($object->getPKey() !== null && !$forceCreate) {
            // object already in DB
            //before save event
            if ($fireSaveEvents) {
            ...
            foreach ($object::getRequiredFields() as $field) {
                if (!array_key_exists($field, $data)) {
                    throw new Exception([
                        "msg" => "[ArrowOrmPersistent] Required value " . $field . " not deliverd to create function.",
                        "values" => $data,
                    ]);
                }
            }
